...
export function ContentProvider({ children }) {
  ...
  const fetchContent = useCallback(async (showLoading = true) => {
    if (!TokenStorage.isAuthenticated() || !isDashboardRoute) {
      setIsLoading(false);
      return;
    }
    try {
      if (showLoading) setIsLoading(true);
      const response = await httpClient.get(ENDPOINTS.CONTENT.GET_ALL, { params: { limit: 100 } });
  ...
  useEffect(() => { fetchContent(); }, [pathname]);

  // Re-export as refetchContents for consumers that need to manually trigger a refresh
  const refetchContents = useCallback(() => fetchContent(false), [fetchContent]);
  ...
  const value = useMemo(
    ...
    [contents, drafts, published, isLoading, addContent, updateContent, deleteContent, publishContent, getContentById, bulkDelete, bulkUpdate, refetchContents]
  );
