    ...
    <div className="bg-white border border-[#E5E7EB] rounded-[20px] p-5 shadow-[0_8px_30px_rgb(0,0,0,0.04)] flex flex-col h-full">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2.5">
          ...
          <h3 className="text-[13px] font-bold text-[#0A0A0F]">{monthName} Goal</h3>
        </div>
        {!editing && (
          <button onClick={() => setEditing(true)} className="p-1 text-neutral-300 hover:text-neutral-600 transition-colors" title="Edit goal">
      ...
      {editing ? (
        <div className="flex items-center gap-2 mb-4">
          <input
            ...
            className="w-20 px-2.5 py-1.5 bg-[#F9FAFB] border border-[#E2E4E9] rounded-lg text-[13px] font-bold outline-none focus:border-emerald-400"
            autoFocus
          />
          <span className="text-[12px] text-neutral-400">uploads / month</span>
          <button onClick={saveGoal} className="ml-auto p-1.5 bg-emerald-500 hover:bg-emerald-600 text-white rounded-lg transition-colors">
        ...
        <div className="mb-4">
          <div className="flex items-baseline gap-1.5">
            <span className="text-[28px] font-[800] text-[#0A0A0F] leading-none tabular-nums">{publishedThisMonth}</span>
