...
import {
  ...
  Loader2, Clock, Calendar, Play, Tag, X,
...
export default function AddContentPage() {
  ...
  useEffect(() => {
    draftRef.current = { step, title, type, platforms, description, references, thumbnails, tags, publishType, scheduleDate, uploadedVideo };
  }, [step, title, type, platforms, description, references, thumbnails, tags, publishType, scheduleDate, uploadedVideo]);
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      ...
      if (!d.title && !d.description) return; // nothing worth saving yet
      try {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (!raw) return;
      const d = JSON.parse(raw);
      if (d.title || d.description) setShowRestoreBanner(true);
    } catch {}
  ...
  useEffect(() => {
    if (editId) {
      ...
      if (urlTags) setTags(urlTags.split(",").map((t) => t.trim()).filter(Boolean));
    }

    // Deep-link support: ?step=1 jumps straight to a given wizard step, in both
    // edit and new-draft modes.
    const stepParam = searchParams.get("step");
    if (stepParam != null) {
      const n = parseInt(stepParam, 10);
      if (!Number.isNaN(n) && n >= 0 && n <= 2) setStep(n);
    }
