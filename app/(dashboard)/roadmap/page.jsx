...
export default function RoadmapPage() {
  ...
  useEffect(() => {
    setIsBrowser(true);
    httpClient.get(ENDPOINTS.ROADMAP.GET_ALL)
      .then(response => {
  ...
  const handleDrop = async (e, colName) => {
    ...
    // Save to DB
    try {
      await httpClient.put(ENDPOINTS.ROADMAP.UPDATE(draggedItemId), {
        id: draggedItemId,
        status: colName,
  ...
  const updateItem = async (id, updates) => {
    ...
    // Save to DB
    try {
      await httpClient.put(ENDPOINTS.ROADMAP.UPDATE(id), { id, ...updates });
    } catch (err) {
  ...
  const handleQuickAdd = async () => {
    ...
    // Save to DB
    try {
      const response = await httpClient.post(ENDPOINTS.ROADMAP.CREATE, newItemTemp);
      setIdeas(prev => prev.map(i => i.id === newItemTemp.id ? response.data : i));
    } catch (err) {
