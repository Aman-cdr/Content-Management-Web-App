...
import {
  ...
  ArrowRight, Clock, Zap, FileText, Target, Flame, RefreshCw,
} from "lucide-react";
...
// Content DB status → Brief Board stage
...
// Brief Board stage advance → Content status to set
...
const MOVE_BACK_STATUS = {
  researching: "IDEA",
  ready:       "IN_PROGRESS",
  archived:    "REVIEW",
};

// Backend platform slug → display name
...
function contentToBrief(c) {
  const rawStatus = c.status;
  // Map back from serialized status (the schema transform outputs lowercase)
  const statusKey = Object.keys(STATUS_TO_STAGE).find(k =>
    k === rawStatus || k.toLowerCase() === rawStatus?.toLowerCase()
  ) || "IDEA";
  const stage = STATUS_TO_STAGE[statusKey] || "idea";

  const platforms = (c.platforms || c.platform || []).map(p =>
    SLUG_TO_DISPLAY[p] || SLUG_TO_DISPLAY[p?.toLowerCase()] || p
  ).filter(Boolean);
  ...
  return {
    ...
    stage,
    platforms,
    hooks:     Array.isArray(c.tags) ? c.tags : [],
    references:[],
    notes:     c.description || "",
...
// Content statuses that belong on the Brief Board (pre-production only)
const BRIEF_STATUSES = new Set(["idea", "draft", "planned", "in_progress", "review", "archived", "IDEA", "PLANNED", "IN_PROGRESS", "REVIEW", "ARCHIVED"]);

export default function BriefBoardPage() {
  const { contents, isLoading, addContent, updateContent, deleteContent } = useContent();
  const [panelOpen, setPanelOpen] = useState(false);
  const [editingBrief, setEditingBrief] = useState(null);

  // Filter to pre-production content only (exclude scheduled/published)
  ...
  const handleSave = useCallback(async (brief) => {
    const platforms = brief.platforms.map(p => DISPLAY_TO_SLUG[p] || "other");
    const payload = {
      title:       brief.title,
      description: brief.notes,
      tags:        brief.hooks,
      platform:    platforms.length ? platforms : ["youtube"],
      contentType: "video",
    };

    if (brief._isNew) {
      await addContent({ ...payload, status: "IDEA" });
    } else {
      await updateContent(brief.id, payload);
    }
  ...
  const handleAdvance = useCallback(async (id) => {
    const brief = briefs.find(b => b.id === id);
    if (!brief) return;
    const newStatus = ADVANCE_STATUS[brief.stage];
    if (newStatus) await updateContent(id, { status: newStatus });
  }, [briefs, updateContent]);

  const handleMoveBack = useCallback(async (id) => {
    const brief = briefs.find(b => b.id === id);
    if (!brief) return;
    const newStatus = MOVE_BACK_STATUS[brief.stage];
    if (newStatus) await updateContent(id, { status: newStatus });
  }, [briefs, updateContent]);
